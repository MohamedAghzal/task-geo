...
def usa_data_cleaner(df):
  ...
  df = df.drop(['totalTestResults', 'dateChecked', 'positiveIncrease', 'negativeIncrease', 
                'totalTestResultsIncrease', 'deathIncrease', 'hospitalizedIncrease', 'hospitalizedCurrently', 'posNeg', 'hospitalizedCumulative'], axis=1)
  df['country'] = 'United States of America'
  df = df.drop_duplicates(subset=['date', 'state', 'positive', 'recovered', 'death'], keep=False)
  ...
  #Renaming columms
  df = df.rename(columns={"state": "state_code", "state_name": "state", "positive": "confirmed", "total":"tested", "death":"fatal"})
  df = df[['date', 'state_code', 'state', 'country', 'confirmed', 'negative', 'inIcuCurrently', 'onVentilatorCurrently' ,
           'recovered',	'fatal', 	'hospitalized', 'tested', 'fips']]
...
def visualize_usa_data(data):
  ...
  data = data.drop(['index', 'tested', 'negative', 'fips'], axis=1) #Remove data irrelevant to the plot
